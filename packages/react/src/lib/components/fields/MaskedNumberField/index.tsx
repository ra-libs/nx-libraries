...
export function MaskedNumberField(
  props: LabeledFieldProps<MaskedNumberFieldProps>,
) {
  ...
  const labelToUse = translate(
    label ? (label as string) : `resources.${resource}.fields.${props.source}`,
  );
  ...
  if (useCurrecyPrefix)
    prefixToUse = MAPPED_CURRENCIES?.[record.currency]?.symbol || '';
  ...
  return useLabeledComponent ? (
    <Labeled label={labelToUse}>{field}</Labeled>
  ) : (
    field
  );
}
